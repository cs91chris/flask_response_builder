...
class DictToXML:
    ...
    def convert_dict(self, obj, ids, parent, attr_type, item_func, cdata):
        ...
        for key, val in obj.items():
            ...
            if isinstance(val, numbers.Number) or type(val) is str:
                ...
                output.append(self.convert_kv(key, val.isoformat(), attr_type, attr, cdata))
                ...
                output.append('<%s%s>%s</%s>' % (
                    key, self._make_attr_string(attr),
                ...
                output.append('<%s%s>%s</%s>' % (
                    key, self._make_attr_string(attr),
                    self.convert_list(val, ids, key, attr_type, item_func, cdata), key
                ))
            elif val is None:
    ...
    def convert_list(self, items, ids, parent, attr_type, item_func, cdata):
        ...
        for i, item in enumerate(items):
            ...
            if isinstance(item, numbers.Number) or type(item) is str:
                ...
                output.append(self.convert_kv(item_name, item.isoformat(), attr_type, attr, cdata))
            elif type(item) is bool:
                output.append(self.convert_bool(item_name, item, attr_type, attr, cdata))
            elif isinstance(item, dict):
                value = self.convert_dict(item, ids, parent, attr_type, item_func, cdata)
                ...
                output.append(tag % (
                    ...
                    self.convert_list(item, ids, item_name, attr_type, item_func, cdata),
                    item_name,
                ))
            elif item is None:
                output.append(self.convert_none(item_name, None, attr_type, attr, cdata))
            else:
    ...
    def convert_kv(self, key, val, attr_type, attr=None, cdata=False):
        ...
        return '<%s%s>%s</%s>' % (
            key, self._make_attr_string(attr),
            self.wrap_cdata(val) if cdata is True else self.escape_xml(val), key
        )
    ...
    def dicttoxml(self, obj, root=True, custom_root='root', ids=False, attr_type=True,
        ...
        if root is True:
            ...
            output.append('<%s>%s</%s>' % (
                custom_root,
                self.convert(obj, ids, attr_type, item_func, cdata, parent=custom_root),
                custom_root,
            ))
        else:
